        ...
        <div className="container mx-auto p-4">
            ...
            {orders.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-80 bg-gray-100 rounded-lg shadow-md">
                    <img
                        src="https://cdn-icons-png.flaticon.com/512/4555/4555977.png" // Placeholder illustration (Change if needed)
                        alt="No Orders"
                ...
                orders.map((order, index) => (
                    <div key={index} className="bg-white p-4 rounded-lg shadow-lg mb-4">
                        ...
                        <p className="text-gray-800 font-semibold">Total Price: ₹{order.totalPrice}</p>

                        {/* Order Items */}
                        <ul className="mt-2">
                            {order.items.map((item, idx) => (
                                <li key={idx} className="border-b border-gray-300 py-2">
                                    {item.productId?.name || "Unknown Product"} - ₹{item.productId?.price || 0} x {item.quantity}
                                </li>
                            ))}
                        </ul>
